...
class _SeekBarPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    void drawSectionText() {
      ...
      double yPosition = sectionTextMarginTop ?? 0; //sectionTextMarginTop
    ...
    void drawDStyleBubble() {
      if (hideBubble! || !alwaysShowBubble!) return;
      paint.color = bubbleColor!;
      double btmX = value! * size.width; //倒三角最下面的点的X轴坐标
      double btmY = -bubbleMargin!; //倒三角最下面的点的Y轴坐标
      Path bubblePath = new Path()
        ..moveTo(btmX, btmY)
        ..lineTo(btmX - 5, btmY - 5)
        ..lineTo(btmX + 5, btmY - 5)
        ..addRRect(RRect.fromRectAndCorners(
            Rect.fromCenter(
                center: Offset(btmX, btmY - 5 - bubbleHeight! / 2),
                width: bubbleWidth!,
                height: bubbleHeight!),
      ...
      //主要是拿到text的宽高，然后给个约束，同时把text准确的放到某个位置上
      ...
      canvas.drawParagraph(
          getParagraph(
              ...
              textSize: textSize),
          Offset(value! * size.width - textSize.width / 2,
              btmY - 5 - bubbleHeight! / 2 - textSize.height / 2));
    }
    ...
    drawDStyleBubble(); //draw top bubble
  }
...
class _SeekBarState extends State<SeekBar> {
  ...
  void initState() {
    ...
    bubbleHeight = widget.bubbleHeight ?? 20;
    bubbleWidth = widget.bubbleWidth ?? 40;
